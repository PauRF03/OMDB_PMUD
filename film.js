...
// Funció per gestionar el clic al checkbox de la watchlist.
function checkboxListener() {
	...
	if ($("#watchlist").is(':checked')) {
		...
		//Crear missatge per indicar que s'ha afegit la pel·licula
		bootstrap.Toast.getOrCreateInstance($("#liveToast")).show()
	} else {
		// Si el checkbox no està marcat, es treu la pel·lícula de watchlist.
		for (let i = 0; i < watchlist.length; i++) if (watchlist[i].id === imdbID) watchlist.splice(i, 1);
	}
